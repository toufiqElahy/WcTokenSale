...
contract('WcToken', function(accounts) {
  var tokenInstance;
  var scaledSupply;

  it('initializes the contract with the correct config values', function() {
    return WcToken.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, 'Wc Token', 'has the correct name');
      return tokenInstance.symbol()
    }).then(function(symbol) {
      assert.equal(symbol, 'Wc', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'Wc Token v1.0', 'has the correct standard');
